...
- **Windows** lists the herdr panes running Claude in the order of the herdr sidebar — worktrees nested under their repo, branch under each window — with their idle time, account, and care mark, and lets the operator set that mark per window (`ever` / `warm` / `off` / back to auto) or click a row to open that window in herdr. The mark file is shared with the `ccmode` shell function. The same two controls also sit on each row of the Usage page's session breakdown, so a window you can already see needs no trip to this tab.
...
Current routes are `/api/health`, `/api/usage`, `/api/burn`, `/api/sessions`, `/api/services`, `/api/conveyor`, `/api/fleet`, and the action routes `POST /api/services/action`, `POST /api/fleet/mode`, and `POST /api/fleet/focus`. Route methods, Origin behavior, body limits, errors, schemas, sorting, and cache behavior belong in [HTTP API](docs/api.md); update it with any route change.
